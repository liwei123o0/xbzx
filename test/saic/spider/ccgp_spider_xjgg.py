...
def ccgpspider():
    ...
    for keyword in keywords:
        urls.append("http://search.ccgp.gov.cn/dataB.jsp?searchtype=1&page_index=1&start_time=2016%3A04%3A06&end_time=2016%3A04%3A13&timeType=7&searchchannel=0&dbselect=bidx&kw={}&bidSort=0&pinMu=0&bidType=2&buyerName=&projectId=&displayZone=&zoneId=&agentName=".format(keyword.replace('\r\n','')))
    for url in urls:
        driver.get(url)
        for i in xrange(1,len(driver.find_elements_by_xpath("//ul[@class='vT-srch-result-list-bid']/li"))+1,1):
            try:
                ...
                zbtype = u"询价公告"
                try:
                    cur.execute("INSERT INTO spider.zb_ccgp_xjgg(url,title,content,zbtype) VALUES ("
